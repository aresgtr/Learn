...
class L22InterceptingLogsSpec extends TestKit(ActorSystem("InterceptingLogsSpec", ConfigFactory.load().getConfig("interceptingLogMessages")))
  with ImplicitSender
  ...
  /**
   * Difficulties with testing this whole flow:
   *   - complex, hard to inject probes
   *   - can't do message based testing
   *     - Checkout actor never sends back any kinds of confirmation outside
   *
   * We intercept logs
   *   - good for integration tests
   */

  import L22InterceptingLogsSpec._
...
object L22InterceptingLogsSpec {

  /* Online store
   *
   *    When you want to send a message to Checkout, Checkout will validate your payment card to the PaymentManager.
   *    PaymentManager will reply to the Checkout with a "accepted" or "denied".
   *    If "accepted", Checkout will talk to the FulfillmentManager.
   *    When Checkout receives from FulfillmentManager, Checkout revert back to normal state to accept other commands.
   */
