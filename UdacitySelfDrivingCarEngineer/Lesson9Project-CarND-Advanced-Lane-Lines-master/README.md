...
The video is generated by slicing the video into frames, and implement the pipeline to analyze each frame. However, searching for lines from scratch is very intensive in computing, and it is expensive to implement for real world self driving cars. To simplify the computation, `find_lane_pixels_from_prior()` method is used. Instead of searching from bottom to the top for the lines, it searches around previous frame's line locations.

A sanity check method is also used to determine if we can search for lines from prior. If we lost track of the lines, or, if the lines that we found does not make sense, we should search from scratch (i.e., from bottom to image top) for the lines.
...
The pipeline appears to be rebust in ideal conditions, even when there is a change in road materials. However, it can still be affected by tree shades, large curves, or cars in front of the camera.
